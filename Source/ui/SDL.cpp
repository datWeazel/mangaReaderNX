...
void drawImage(SDL_Texture *texture, int x, int y, double angle)
{
    SDL_Rect pos = {pos.x = x, pos.y = y};
    SDL_RendererFlip flip = SDL_FLIP_NONE;
    SDL_QueryTexture(texture, NULL, NULL, &pos.w, &pos.h);
    SDL_RenderCopyEx(main_renderer, texture, NULL, &pos, angle, NULL, flip);
}

void drawImageScale(SDL_Texture *texture, int x, int y, int w, int h, double angle)
{
    SDL_Rect pos = {pos.x = x, pos.y = y, pos.w = w, pos.h = h};
    SDL_RendererFlip flip = SDL_FLIP_NONE;
    SDL_Point rotatePoint = {0, 0};
    SDL_RenderCopyEx(main_renderer, texture, NULL, &pos, angle, &rotatePoint, flip);
}
